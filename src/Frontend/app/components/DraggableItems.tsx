...
export default function DraggableItems({ droppedItems } : { droppedItems: IDroppedItem[] }) {
    return (
        <div className='flex justify-start items-center px-12 py-3 w-full h-[30%] border-b-2 border-[#EFEFEF] gap-12'>
            ...
            <DraggableItem icon={Apertar} commandName='Apertar' pairableItems={['Quadrante']} lastDroppedItem={droppedItems[droppedItems.length - 1] || null} />
            <DraggableItem icon={Quadrante} commandName='Quadrante' pairableItems={['Mostrar imagem', 'Tocar som']} lastDroppedItem={droppedItems[droppedItems.length - 1] || null} />
